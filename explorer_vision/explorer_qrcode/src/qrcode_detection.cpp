    ...
    : nh_(nh), image_transport_(nh_), listener_(0)
{
  ...
  //priv_nh.param("sub_image_qrcode_topic_camera1", sub_image_qrcode_topic_camera1_, std::string("/camera/color/image_raw/compressed"));
  ...
  //priv_nh.param("camera_info1", camera_info1_, std::string("/camera/color/camera_info"));
  ...
  priv_nh.param("camera_frame",camera_frame_,std::string("camera_left"));
  //灰度图像显示的场景
...
void qrcode_detection_impl::imageCallback(const sensor_msgs::CompressedImageConstPtr& image)
{
 ...
 //percept.camera_info=*left;
  ...
  // !!!!!!!!!!!!!用QR码来代替victim，发布的消息，只有image_percept不一样!!!!!!!!!!!!
...
//rs的frame_id貌似是captical_frame，目前暂定
{
        //ROS_ERROR("in");
      try
      {
