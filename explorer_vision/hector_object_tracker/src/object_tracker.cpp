...
cv::Point3d direction_cv;
void ObjectTracker::MsgDisposal(const geometry_msgs::Point::ConstPtr& msg)
{
    direction_cv.x=msg->x;
    direction_cv.y=msg->y;
    direction_cv.z=msg->z;
    return;
}
...
ObjectTracker::ObjectTracker()
{
 ...
 ROS_ERROR("In object_tracker.cpp     In ObjectTracker.. 1"); //5
  ...
  ros::Subscriber sub_temp = temp.subscribe("camera/rs/qr_point", 1, &ObjectTracker::MsgDisposal,this);
 ...
 */
 ...
 ROS_ERROR("++++++=====+++++ imagePerceptCb=======MID");
  ...
  ROS_ERROR("--> Rectified image coordinates: [%f,%f]", rectified.x, rectified.y);
  ROS_ERROR("--> Projected 3D ray (OpenCV):   [%f,%f,%f]", direction_cv.x, direction_cv.y, direction_cv.z);
  ROS_ERROR("--> Projected 3D ray (tf):       [%f,%f,%f]", pose.getOrigin().x(), pose.getOrigin().y(),pose.getOrigin().z());
 //DEBUG can not appeal
...
//所有QR点相对于map的世界坐标存储在该变量中
//变量类型为hector_worldmodel_msgs::ObjectModel（自定义数据类型）
