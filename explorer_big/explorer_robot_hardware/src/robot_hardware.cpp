      ...
      last_right_wheel_vel_(0.0)
{
    ...
    //overwrite the explorer_hardware_config.yaml
    ...
    /**(!this->getNameList(this->nh_, "/explorer_vice_wheel_controller/front_vice_wheel", vice_wheel_joint_name_) ||
    ...
    else
    {
        ...
        this->getNameList(this->nh_, "explorer_vice_wheel_controller/joints", vice_wheel_joint_name_);
    ...
    **/
    ...
    for (auto name : arm_joint_name_)
    {
        ...
        //将从joint_state接受到的消息放到joint_pose
...
void ExplorerHardware::pub_vice_wheel_cmd()
{
    ...
    if (vice_wheel_cmd_["left_up_fin_base_joint"] != 0.0 || vice_wheel_cmd_["right_up_fin_base_joint"] != 0.0)
    {
        ...
        //vice_wheel_message.data.push_back(-(vice_wheel_pos_["left_up_fin_base_joint"]*180/PI));
        //vice_wheel_message.data.push_back(-(vice_wheel_pos_["right_up_fin_base_joint"]*180/PI));
   ...
   if (vice_wheel_cmd_["left_down_fin_base_joint"] != 0.0 || vice_wheel_cmd_["right_down_fin_base_joint"] != 0.0)
    {
        ...
        //vice_wheel_message.data.push_back(vice_wheel_pos_["left_down_fin_base_joint"]*180/PI);
        //vice_wheel_message.data.push_back(vice_wheel_pos_["right_down_fin_base_joint"]*180/PI);
...
void ExplorerHardware::pub_arm_joint_cmd()
{
    ...
    for (auto name : arm_joint_name_)
    {
        ...
        arm_joint_pos_[name] += arm_joint_cmd_[name]; //-------------------------------可能需要修改 此处是pos+cmd 也就是相对角度
        arm_joint_vel_[name] = 0.0;
        arm_joint_eff_[name] = 0.0;
        //arm_joint_cmd_[name] = 0.0;
    }
